 ...
 * @date 2019-11-11
 */
public class SqlRowTypeTest extends BaseParser {
    private final static Logger LOG = LoggerFactory.getLogger(SqlRowTypeTest.class);

    @Test
    public void createKafkaSourceWithRowType() {
        String sql =
                " CREATE TABLE table_source(\n" +
                        "    name varchar,\n" +
                        "    channel string,\n" +
                        "    r row<f0 int not null,f1 boolean>,\n" +
                        "    pv int,\n" +
                        "    xctime bigint\n" +
                        " )WITH(\n" +
                        "    type='kafka11',\n" +
