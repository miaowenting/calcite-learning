 ...
 * @date 2019-11-11
 */
public class SqlMapTypeTest extends BaseParser {
    private final static Logger LOG = LoggerFactory.getLogger(SqlMapTypeTest.class);

    @Test
    public void createKafkaSourceWithMapType() {
        String sql =
                " CREATE TABLE table_source(\n" +
                        "    name varchar,\n" +
                        "    channel string,\n" +
                        "    m map<bigint,boolean>,\n" +
                        "    pv int,\n" +
                        "    xctime bigint\n" +
                        " )WITH(\n" +
                        "    type='kafka11',\n" +
